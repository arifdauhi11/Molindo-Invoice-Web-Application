      ...
      <div class="content-wrapper">
        <div class="content-header row">
          <div class="content-header-left col-md-8 col-12 mb-2 breadcrumb-new">
            <h3 class="content-header-title mb-0 d-inline-block">Pemasukkan</h3>
            <div class="row breadcrumbs-top d-inline-block">
              <div class="breadcrumb-wrapper col-12">
                <ol class="breadcrumb">
                  <li class="breadcrumb-item"><a href="<?php echo site_url('dashboard') ?>">Dashboard</a>
                  </li>
                  <li class="breadcrumb-item active">Pemasukkan
                  </li>
                </ol>
              </div>
            </div>
            ...
            <div class="<?php if($this->session->userdata('role') == 'Kolektor' || $this->session->userdata('role') == 'Direktur'){ 
              ...
              <div class="card">
                <div class="card-header">
                    ...
                    <h4 class="card-title">Data Pemasukkan <?php echo $thn ?></h4>
                    <a class="heading-elements-toggle"><i class="fa fa-ellipsis-v font-medium-3"></i></a>
                    <br>
                    <?php if ($this->session->userdata('role') == 'Finance') { ?>                      
                    <a class="btn btn-blue" href="<?php echo site_url('pemasukkan/add') ?>"><i class="ft-plus-square"></i> Tambah Pemasukkan</a>
                    <?php } ?>
                </div>
                      ...
                      <div class="text-center">                        
                        ...
                        <?php } else { ?>                          
                          <h4 id="blockquoteId">
                            <blockquote>
                              <?php if ($bulan) {
                                echo "Belum ada data pemasukkan tahun ".$thn." bulan ".$bulan." ".$tahap;
                              } else {
                                echo "Belum ada data pemasukkan tahun ".$thn.".";
                              }  ?>                              
                            </blockquote></h4>
                          <br>                          
                          <h4><blockquote>Pilih tahun lainnya :</blockquote></h4>
            ...
            <div class="col-lg-6 col-md-12 mt-3">
              ...
              <?php } ?>                      
                    <div class="card-content">
                      <center>
                        <img class="mt-1 card-img-top img-fluid" width="100%" height="160px" src="<?php echo base_url('assets/images/kwitansi/'.$key->bukti_kwitansi) ?>" alt="Card image cap">
                        </center>
                        <div class="card-body">
                            <h4 class="card-title text-center" style="font-size: 16px"><?php echo $key->masuk ?></h4>
                    ...
                    <div class="card-footer">                        
                        ...
                        <a class="btn btn-warning btn-sm" href="<?php echo site_url('pemasukkan/edit/'.$key->id_anggaran) ?>"><i class="ft-edit"></i> Edit</a>
                        <button class="float-right btn btn-danger btn-sm" onclick="hapusData('<?php echo $key->id_anggaran?>','<?php echo $key->masuk?>','pemasukkan/hapus/')"><i class="ft-trash"></i> Hapus</button>
                        <?php } else { ?>
                        <button class="btn btn-warning btn-sm" onclick="editP('<?php echo $key->id_anggaran?>','<?php echo $key->masuk.' '.$key->tahap.' bulan '.$key->bulan.' tahun '.$key->tahun_anggaran?>','<?php echo $userData->nama_user ?>')"><i class="ft-edit"></i> Edit</button>
                        <button class="float-right btn btn-danger btn-sm" onclick="hapusP('<?php echo $key->id_anggaran?>','<?php echo $key->masuk.' '.$key->tahap.' bulan '.$key->bulan.' tahun '.$key->tahun_anggaran?>','<?php echo $userData->nama_user ?>')"><i class="ft-delete"></i> Hapus</button>
                        <?php } ?>
    ...
    <script type="text/javascript">           
      function getByTahun(tahun) {
        if (tahun.length > 4 || tahun.length < 4) {
          ...
          window.location = '<?php echo base_url('pemasukkan/?tahun=') ?>'+tahun;
        }
      }
      ...
      const tahap = urlParams.get('tahap')                       
      ...
      function getByFilter() {          
        ...
        if($('[name="radioFilter"]').is(':checked')){
          if($('[name="radioFilterTahap"]').is(':checked')){            
            window.location = '<?php echo base_url('pemasukkan/?tahun=') ?>'+tahun+'&bulan='+radioValue+'&tahap='+radioValueTahap;
          }else{
      ...
      function hapusP(id,namaData,namaKaryawan) {
        swal({
            ...
            buttons: {
                ...
                if (isConfirm) {
                  $.ajax({
                   url: "<?php echo site_url()?>/notifikasi/hapusByKaryawan/"+id+"/"+namaData+'/'+namaKaryawan+'/pemasukkan',
                   type: "POST",
                   dataType: "JSON",
      ...
      function editP(id,namaData,namaKaryawan) {
        $.ajax({
         url: "<?php echo site_url()?>/notifikasi/editByKaryawan/"+id+"/"+namaData+'/'+namaKaryawan+'/pemasukkan',
         type: "POST",
         dataType: "JSON",
         success: function(data) {
          swal({
