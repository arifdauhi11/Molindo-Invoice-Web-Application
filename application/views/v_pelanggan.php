            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header">
                    ...
                    <?php if ($this->session->userdata('role') == 'Finance' || $this->session->userdata('role') == 'Customer Service') { ?>                      
                    <button class="btn btn-info btn-sm mt-1" onclick="tambahData()">Tambah Data Pelanggan</button>
                    <?php } ?>
                </div>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                      ...
                                      <a class="btn btn-primary btn-square btn-sm" href="<?php echo site_url('invoice/add/'.$key->id_pelanggan); ?>"><i class="ft-clipboard"></i> Buat Invoice</a> |                                       
                                      <?php } ?>                                      
                                      <a class="btn btn-secondary btn-square btn-sm" href="<?php echo site_url('invoice/allById/'.$key->id_pelanggan) ?>"><i class="ft-clipboard"></i> Semua Invoice</a> | 
                                      ...
                                      <?php if ($this->session->userdata('role') == 'Direktur') { ?>
                                        |
                                      <a class="btn btn-warning btn-square btn-sm" href="<?php echo site_url('pelanggan/edit_pelanggan/'.$key->id_pelanggan) ?>"><i class="ft-edit"></i> Edit</a>
                                       |                                         
                                      <?php } else if ($this->session->userdata('role') == 'Finance') { ?>
                                       |
                                      <button class="btn btn-warning btn-square btn-sm" onclick="editP('<?php echo $key->id_pelanggan?>','<?php echo $key->nama_pelanggan?>','<?php echo $userData->nama_user ?>')"><i class="ft-edit"></i> Edit</button>
                                       | 
                                      <?php } ?>
                                      <?php if ($this->session->userdata('role') == 'Direktur' || $this->session->userdata('role') == 'Finance') { ?>
                                      <button class="btn btn-danger btn-square btn-sm" onclick="putus('<?php echo $key->id_pelanggan ?>','<?php echo $key->nama_pelanggan ?>')"><i class="ft-delete"></i> Hapus</button>
                                      <?php } ?>                                      
                                      <?php } ?>                                        
                                    </td>
                                </tr>                                  
    ...
    <script type="text/javascript">
      ...
      function hapusPL(id,namaPL,namaKaryawan) {
        swal({
            ...
            buttons: {
                ...
                if (isConfirm) {
                  $.ajax({
                   url: "<?php echo site_url()?>/notifikasi/hapusByKaryawan/"+id+"/"+namaPL+'/'+namaKaryawan,
                   type: "POST",
                   dataType: "JSON",
                   success: function(data) {
                    swal({
      ...
      function putus(id,namaPL) {
        swal({
            title: "Anda yakin?",
            text: "Data "+namaPL+" yang anda hapus akan dipindahkan ke pelanggan putus.\n Anda dapat mengaktifkan kembali jika diperlukan.",
            icon: "warning",
            showCancelButton: true,
            buttons: {
                        cancel: {
                        ...
                        confirm: {
                            text: "Ya, hapus sementara data ini!",
                            value: true,
                            visible: true,
                            className: "",
                            closeModal: false
                ...
                if (isConfirm) {
                  $.ajax({
                   url: "<?php echo site_url()?>/pelanggan/putus_action/"+id+"/"+namaPL,
                   type: "POST",
                   dataType: "JSON",
                   success: function(data) {                    
                    if (data.status == true) {
                      swal({
                        title: "Sukses",
                        text: "Date telah dihapus sementara.",
                        icon: "success",                 
                        buttons : {
                          cancel: {
                              text: "Tidak",
                        ...
                        if (isConfirm) {
                          window.location = '<?php echo site_url('pelanggan/putus') ?>';
                        } else {
                          location.reload();
                        }
                      });                                          
      ...
      function editP(id,namaData,namaKaryawan) {
        $.ajax({
         url: "<?php echo site_url()?>/notifikasi/editByKaryawan/"+id+"/"+namaData+'/'+namaKaryawan+'/pelanggan',
         type: "POST",
         dataType: "JSON",
         success: function(data) {
          swal({
